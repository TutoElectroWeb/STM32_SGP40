 ...
 * - Cet exemple utilise le chemin CALLBACK (SetCallbacks + Process) — les données
 *   sont consommées dans SGP40_App_OnDataReady appelée par SGP40_Async_Process().
 *   Le chemin POLLING (SGP40_Async_HasData + SGP40_Async_GetData + SGP40_Async_IsIdle)
 *   est disponible en alternative ; il est encapsulé dans SGP40_Async_Tick().
...
int main(void)
{
  ...
  while (1)                                                            // Boucle principale infinie de supervision async IT
  {
      ...
      uint16_t voc_raw = 0U;
      SGP40_TickResult tick = SGP40_Async_Tick(&sgp40_async, now, &voc_raw); // Pattern A : avance FSM + retourne état
      switch (tick)
      {
        case SGP40_TICK_DATA_READY:
          /* traitement applicatif déjà effectué dans SGP40_App_OnDataReady */
          break;
        case SGP40_TICK_ERROR:
          ...
          break;
        case SGP40_TICK_IDLE:
        case SGP40_TICK_BUSY:
        default:
          break;
      }
